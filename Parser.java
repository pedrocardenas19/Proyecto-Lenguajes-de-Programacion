...
public class Parser{
	...
	**/
	...
	public void comparation(){
		expr();
		if(lexer.getCurrentToken().code ==Lexer.DIFFERENT){
			recognize(Lexer.DIFFERENT);
		}
		else if (lexer.getCurrentToken().code ==Lexer.EQUALS){
	...
	public void factor(){
		if (lexer.getCurrentToken().code == Lexer.VARIABLE){
			...
			recognizeConstant();
		else if (lexer.getCurrentToken().code != Lexer.ADDITION && lexer.getCurrentToken().code != Lexer.MULTIPLY){ 
			recognize(Lexer.LPAREN);
			expr();
			recognize(Lexer.RPAREN);
		}
